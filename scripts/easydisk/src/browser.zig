...
const help = struct {
    const keys = [_][:0]const u8{
        ...
        "right/enter", "Open selected directory",
              "space", "Open selected item in Finder",
...
pub fn keyInput(ch: i32) void {
    ...
    switch (ch) {
        ...
        ' ' => {
            if (dir_items.items.len > 0) {
                const item = dir_items.items[cursor_idx];
                const path = if (item) |e|
                    std.fs.path.joinZ(main.allocator, &[_][]const u8{ dir_path, e.name() }) catch unreachable
                else
                    main.allocator.dupeZ(u8, dir_path) catch unreachable;
                defer main.allocator.free(path);
                ui.openInFinder(path);
            }
        },
        10, 'l', c.KEY_RIGHT => {
            if (dir_items.items.len == 0) {
